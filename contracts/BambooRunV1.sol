...
contract BambooRunV1 is ERC721Upgradeable {
    // Royalty
    ...
    // map id to BambooRun obj
    mapping(uint256 => BambooRun) public allBambooRun;
    ...
    event SaleToggle(uint256 bambooNumber, bool isForSale, uint256 price);
    event PurchaseEvent(
        uint256 bambooNumber,
        address from,
        address to,
        uint256 price
    );
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            BambooRun memory newBambooRun = BambooRun(
                newItemId,
                msg.sender,
                msg.sender,
                0,
            ...
            // add the token id to the allBambooRun
            allBambooRun[newItemId] = newBambooRun;
        }
    }
    ...
    function setPriceForSale(
        ...
        BambooRun memory Bamboo = allBambooRun[_tokenId];
        Bamboo.price = _newPrice;
        Bamboo.forSale = isForSale;
        allBambooRun[_tokenId] = Bamboo;
    ...
    function getAllSaleTokens() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _totalSupply; i++) {
            if (allBambooRun[i].forSale == true) {
                _tokenForSales[counter] = allBambooRun[i].tokenId;
                counter++;
            }
        }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        // get that token from all BambooRun mapping and create a memory of it defined as (struct => BambooRun)
        BambooRun memory Bamboo = allBambooRun[_tokenId];
        // price sent in to buy should be equal to or more than the token's price
        require(msg.value >= Bamboo.price);
        // token should be for sale
        require(Bamboo.forSale);
        ...
        payable(Bamboo.currentOwner).transfer(payOwnerAmount);
        Bamboo.previousPrice = Bamboo.price;
        allBambooRun[_tokenId] = Bamboo;
    ...
    function _beforeTokenTransfer(
        ...
        BambooRun memory Bamboo = allBambooRun[tokenId];
        Bamboo.currentOwner = to;
        Bamboo.numberOfTransfers += 1;
        Bamboo.forSale = false;
        allBambooRun[tokenId] = Bamboo;
        if (from == address(0)) {
